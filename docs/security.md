...
- **Pepper**: An optional server-wide pepper (`EXPOSE_API_KEY_PEPPER`) is appended before hashing. If not configured, one is auto-generated at startup
...
- **Sensitive field privacy**: Password-like form/JSON fields are inspected like any other input, but their values are never written to logs — the WAF logs only the matched rule name, method, URI path, remote address, and User-Agent. Excluding these fields from scanning was previously used to reduce false positives, but it was trivially bypassed by naming an attack parameter `password`. The trade-off is that a password containing rule-matching text (`' OR 1=1`, `<script`) can now cause the login request to be blocked
- **Body inspection limits**: Only the first N bytes (configurable via `EXPOSE_WAF_BODY_INSPECT_LIMIT`, default 16 KiB) are scanned. Bodies that are not valid UTF-8 are treated as binary and skipped. Multipart bodies are parsed and each part's field name, filename, and textual content are scanned — a file upload whose first 16 KiB contains rule-matching content (an `.html`, `.js`, or `.sql` file, for example) can therefore be blocked. Use `wafIgnorePaths` on the tunnel to exempt upload endpoints
...
The server injects standard reverse-proxy headers (`X-Forwarded-For`, `X-Forwarded-Proto`, `X-Forwarded-Host`, `X-Forwarded-Port`). For `X-Forwarded-For`, a client-supplied chain is preserved and the immediate peer's IP is appended as the rightmost hop — this lets an upstream trusted proxy's chain pass through while still recording who connected to expose.
...
Release artifacts are signed with [cosign](https://docs.sigstore.dev/) in **keyless mode**: the release workflow authenticates to Sigstore via GitHub Actions OIDC (`id-token: write`), so there are no long-lived signing keys to manage or leak. GoReleaser signs `checksums.txt`, producing two additional release assets:

- `checksums.txt.sig` — the signature over `checksums.txt`
- `checksums.txt.pem` — the signing certificate, bound to the workflow identity
